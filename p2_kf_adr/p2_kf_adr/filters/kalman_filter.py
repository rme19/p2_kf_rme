...
class KalmanFilter:
    ...
    def predict(self, u, dt):
        ...
        #Actualizamos las matrices A y B
        A=self.A()
        B=self.B(self.mu,dt)

        self.mu = A @ self.mu + B @ u
        self.Sigma = A @ self.Sigma @ np.transpose(A) + self.R
    ...
    def update(self, z):
        ...
        K = self.Sigma @ np.transpose(self.C) @ np.linalg.inv(self.C@self.Sigma@np.transpose(self.C)+self.Q)
